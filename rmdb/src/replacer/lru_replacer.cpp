...
void LRUReplacer::pin(frame_id_t frame_id) {
    ...
    // 固定指定id的frame
    // 在数据结构中移除该frame
    auto hash_it = LRUhash_.find(frame_id);

    if (hash_it == LRUhash_.end()) {
        return ;
    }

    auto list_it = hash_it->second;
    LRUlist_.erase(list_it);
    LRUhash_.erase(hash_it);
}
...
void LRUReplacer::unpin(frame_id_t frame_id) {
    ...
    if (hash_it == LRUhash_.end()) {
        ...
        LRUhash_.insert({frame_id, LRUlist_.begin()});
        return ;
    }

//    auto list_it = hash_it->second;
//    auto old_fid = *(list_it);
//    LRUlist_.erase(list_it);
//    LRUlist_.emplace_front(old_fid);
//    hash_it->second = LRUlist_.begin();
}
